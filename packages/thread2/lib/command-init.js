...
var commandInit = {
    ...
    execute: function (opts) {
        ...
        if (!opts.projectName) {
            utils.log(['error: 请设置要初始化的项目名称. `thread2 <projectName>`'])
            return
        }

        if (!opts.theme) {
            utils.log(['error: 请指定要使用的项目框架. 如 `thread2 -t react-mobx`. 如果不确定, 可以运行 `thread2 list-theme` 查看可用 theme'])
            return
        }

        if (config.path.projectRoot !== null) {
            utils.log(['error: 请不要在一个 thread2 seed 的项目内再创建一个 thread2 seed 项目.'])
            return
        }

        var targetProjectPath = utils.p(config.path.cwd + '/' + opts.projectName)
        var seedPath = utils.p(targetProjectPath + '/_thread2_/seed')
        ...
        return sp('git clone ' + opts.remote + ' ' + opts.projectName, spCwd)
            ...
            .then(function () {
                utils.logs([
                    ['info: thread2 seed 新项目已创建, 请进入该文件夹, 运行 `npm install` 然后 运行 `thread2 dev` 开始开发. ']
                ])
            })
            .catch(function (ex) {

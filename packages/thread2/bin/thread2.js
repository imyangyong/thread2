...
if (config.path.projectRoot !== null) {
    try {
        require(utils.p(config.path.projectRoot + '/link/thread2-command-line.js'))({
            commander: commander
        })
    } catch(ex) {
        utils.log(['出现错误 : ', ex])
        utils.log(['warn: 是否 thread2 seed 项目还没有初始化, 因此有一些命令还不能执行? 如: dev, dest 等, 如要运行, 请 `npm install` 后再试'])
    }
}
